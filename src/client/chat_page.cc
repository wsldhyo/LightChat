...
ChatPage::ChatPage(QWidget *parent)
  ...
  ui->setupUi(this);
  //设置按钮样式
  ui->receive_btn->set_state("normal", "hover", "press");
  ui->send_btn->set_state("normal", "hover", "press");
...
void ChatPage::set_user_info(std::shared_ptr<UserInfo> user_info) {
  ...
  ui->chat_data_list->remove_all_item();
  // 再逐条添加与该好友的聊天记录条目，TODO，只添加视图内的条目
  for (auto &msg : user_info->chat_msgs_) {
    append_chat_msg(msg);
  }
}
...
void ChatPage::append_chat_msg(std::shared_ptr<TextChatData> msg) {
  ...
  if (msg->from_uid_ == self_info->uid_) {
    ...
    pChatItem->set_user_name(self_info->name_);
    pChatItem->set_user_icon(QPixmap(self_info->icon_));
    QWidget *pBubble = nullptr;
    pBubble = new TextBubble(role, msg->msg_content_);
    pChatItem->set_widget(pBubble);
    ui->chat_data_list->append_chat_item(pChatItem);
  } else {
    role = ChatRole::OTHERS;
    ChatItemBase *pChatItem = new ChatItemBase(role);
    auto friend_info =
        UserMgr::get_instance()->get_friend_infO_by_id(msg->from_uid_);
    if (friend_info == nullptr) {
      return;
    }
    pChatItem->set_user_name(friend_info->name_);
    pChatItem->set_user_icon(QPixmap(friend_info->icon_));
    QWidget *pBubble = nullptr;
    pBubble = new TextBubble(role, msg->msg_content_);
    pChatItem->set_widget(pBubble);
    ui->chat_data_list->append_chat_item(pChatItem);
...
void ChatPage::on_send_btn_clicked() {
  ...
  QString userName = self_info->name_;
  QString userIcon = self_info->icon_;

  const QVector<MsgInfo> &msgList = pTextEdit->get_msg_list();
  QJsonObject textObj;
  ...
  for (int i = 0; i < msgList.size(); ++i) {
    ...
    ChatItemBase *pChatItem = new ChatItemBase(role);
    pChatItem->set_user_name(userName);
    pChatItem->set_user_icon(QPixmap(userIcon));
    QWidget *pBubble = nullptr;
    ...
    if (type == "text") {
      ...
      if (txt_size + msgList[i].content.length() > 1024) {
        textObj["fromuid"] = self_info->uid_;
        textObj["touid"] = user_info_->uid_;
      ...
      auto txt_msg =
          std::make_shared<TextChatData>(uuidString, obj["content"].toString(),
                                         self_info->uid_, user_info_->uid_);
