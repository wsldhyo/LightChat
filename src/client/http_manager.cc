...
void HttpMgr::post_http_req(QUrl url, QJsonObject json, ReqId req_id,
                            Modules mod) {
  ...
  QObject::connect(
      reply, &QNetworkReply::finished, [reply, self, req_id, mod]() {
        ...
        //发送信号通知http请求已响应
        emit self->sig_http_finish(req_id, res, ErrorCodes::NO_ERROR, mod);
        reply->deleteLater();
        return;
      });
}
...
void HttpMgr::create_connection() {
