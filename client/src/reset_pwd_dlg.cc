...
ResetPwdDlg::ResetPwdDlg(QWidget *_parent /*nullptr*/)
  ...
  // 连接reset相关信号和注册处理回调
  init_http_handlers();
  connect(HttpManager::get_instance().get(), &HttpManager::sig_http_finished,
          this, &ResetPwdDlg::slot_reset_mod_finished);
}
...
void ResetPwdDlg::slot_return_code_btn_clicked() {
...
void ResetPwdDlg::slot_confirme_btn_clicked() {
  ...
  json_obj["passwd"] = res;
  json_obj["varifycode"] = ui_->vertify_code_edit->text();
  HttpManager::get_instance()->post_http_request(
      QUrl(gate_url_prefix + "/reset_pwd"), json_obj, RequestID::RESET_PWD,
      Modules::RESET_PWD_MOD);
}
void ResetPwdDlg::slot_reset_mod_finished(RequestID _req_ID, QString _res,
                                          ErrorCode _err) {
