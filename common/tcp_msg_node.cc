...
TcpSendNode::TcpSendNode(const char *msg, short _max_len, short _msg_id)
    : TcpMsgNode(TCP_MSG_HEAD_LENGTH), msg_id_(_msg_id) {
  // 先发送id, 转为网络字节序
  short msg_id_host =
      boost::asio::detail::socket_ops::host_to_network_short(msg_id_);
  memcpy(data_, &msg_id_host, TCP_MSG_ID_LENGTH);
  // 转为网络字节序
  short max_len_host =
      boost::asio::detail::socket_ops::host_to_network_short(_max_len);
  memcpy(data_ + TCP_MSG_ID_LENGTH, &max_len_host, TCP_MSG_BODY_LENGTH);
  memcpy(data_ + TCP_MSG_ID_LENGTH + TCP_MSG_BODY_LENGTH, msg, _max_len);
}
