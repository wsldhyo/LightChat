...
void Server::start() {
  ...
  acceptor_.async_accept(new_connection->get_socket(), [new_connection, self](beast::error_code _ec) {
    try {
      ...
      std::cout<< "accept: " << new_connection->get_socket().remote_endpoint().address().to_string() << std::endl;
